<template>
  <article
    ...
    class="service-window absolute flex flex-col overflow-hidden rounded-lg border border-black/20 bg-black/20 text-slate-150 shadow-[0_30px_70px_rgba(2,6,23,0.36)] backdrop-blur-2xl transition-[box-shadow,transform,opacity] duration-200"
    :class="window.active ? 'ring-1 ring-sky-900/30' : 'scale-[0.995] opacity-95'"
    :style="windowStyle"
    @pointerdown="windowManager.focusWindow(window.id)"
  >
    <header
      class="flex h-14 items-center justify-between border-none border-slate-800/80 bg-black/20 px-4 backdrop-blur-xl"
    ...
    <div class="relative flex-1 overflow-hidden bg-black/20">
      <template v-if="window.service.embed">
        ...
        >
          <div class="text-center">
            <div class="mx-auto h-10 w-10 animate-pulse rounded-full bg-sky-900"></div>
        ...
        >
          <div class="max-w-md rounded-[1.5rem] bg-white/92 p-6 text-center shadow-xl">
            ...
            <h2 class="mt-4 text-lg font-semibold text-slate-200">
            ...
            <div class="mt-5 flex flex-wrap justify-center gap-3">
              <button
                type="button"
                class="rounded-2xl bg-slate-950 px-4 py-2 text-sm font-semibold text-white! color-white"
                @click="reloadEmbed"
              >
                Riprova
              </button>
              <a
                ...
                target="_blank"
                rel="noreferrer"
                class="rounded-2xl border border-slate-200 px-4 py-2 text-sm font-semibold text-slate-200!"
              >
                Apri esternamente
              </a>
            </div>
      ...
      <div
        v-else
        class="grid h-full place-items-center bg-slate-800 p-8 text-center"
      >
        <div class="flex flex-col items-start">
          <IconBubble :service="window.service" />
          <h2 class="mt-5 text-2xl font-semibold">{{ window.service.name }}</h2>
          <p class="mx-auto mt-2 max-w-sm text-sm text-slate-300">
